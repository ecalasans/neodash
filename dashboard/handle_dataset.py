'''
Módulo para manipulação do dataset Redcaprn
...
def getDfs():
    ...
    if df is not None:
        ...
        admissao = df[['data_adm', 'indic_adm', 'id_adm', 'peso_adm', 'est_adm', 'pc_adm',
        ...
        card = df[['pca', 'med_pca', 'med_util_pca', 'cc', 'cir_cardiaca', 'cvu', 'cva',
                   'picc', 'dias_dissec', 'pun_ven_prof', 'dva', 'eco']]

        neuro = df[['usg_tf', 'outros_usgtf', 'hidrocefalia', 'leucomalacia', 'convulsao',
                    'anticonvulsivante', 'eeg', 'eeg_alterado']]

        oftalmo = df[['rop', 'grau_rop', 'rop_plus', 'zona_avascular',
                      'cirurgia_rop', 'refl_vermelho']]

        hemato = df[['incomp_sang', 'ictericia', 'max_bt', 'fototerapia', 'dias_foto',
        ...
        renal = df[['ins_renal', 'max_ureia', 'max_creatinina']]

        infecto = df[['sepse_precoce', 'hmc_admissao', 'hmc_adm_res', 'sepse_tardia',
        ...
        atb = df[['ampicilina', 'genta', 'cefepime', 'amica', 'meropenem', 'vanco',
        ...
        imuno = df[['v_hep_b', 'v_bcg', 'palivizumabe', 'imunoglobulina']]

        metab = df[['dist_glicose', 'min_glicose', 'max_glicose', 'dist_sodio', 'min_na',
        ...
        cir = df[['pntx', 'pneumoperit', 'pnmed', 'cir_abd', 'retinopexia', 'hernia_ing',
                  'dvp_dve', 'malf_cong', 'tipo_anom_cong']]

        nut = df[['min_peso', 'id_peso_min', 'max_peso', 'id_pn', 'peso_28',
                  'id_dieta_enteral', 'lme_lmo', 'fm85', 'colostroterapia', 'formula_art',
                  'transicao_sonda_sm', 'idade_smld']]

        desfecho = df[['desfecho', 'loc_obito', 'causa_obito', 'data_alta', 'readmissao',
...
def getAntMaternos():
    ...
    ant_maternos = dict_dfs['ant_maternos']

    # Fumo
    ant_maternos['fumo'] = ant_maternos['fumo'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Alcoolismo
    ant_maternos['alcool'] = ant_maternos['alcool'].replace(
        ['1', '2', '3', ''], ['Sim', 'Não', 'Desconhecido', 'Sem registro']
    )

    # Frequencia do uso de álcool
    ant_maternos['freq_alcool'] = ant_maternos['freq_alcool'].replace(
        ['1', '2', ''], ['Sim', 'Não', 'Sem registro']
    )

    # Uso de drogas psicoativas
    ant_maternos['psicoativas'] = ant_maternos['psicoativas'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Violência contra gestante
    ant_maternos['violencia'] = ant_maternos['violencia'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Corioamnionite
    ant_maternos['corioamnionite'] = ant_maternos['corioamnionite'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Retardo do crescimento intra-uterino
    ant_maternos['rcui'] = ant_maternos['rcui'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Trabalho de parto prematuro
    ant_maternos['tpp'] = ant_maternos['tpp'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Descolamento prematuro da placenta
    ant_maternos['dpp'] = ant_maternos['dpp'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Hipertensão arterial materna prévia
    ant_maternos['has'] = ant_maternos['has'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Doença hipertensiva específica da gravidez
    ant_maternos['dheg'] = ant_maternos['dheg'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Síndrome HELLP
    ant_maternos['hellp'] = ant_maternos['hellp'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Uso de sulfato de magnésio para eclâmpsia/HELLP
    ant_maternos['sulfatada'] = ant_maternos['sulfatada'].replace(
        ['0', '1', '2', '3', ''], ['Não', 'Sim', 'Não', 'Desconhecido', 'Sem registro']
    )

    # Oligoâmnio
    ant_maternos['oligoamnio'] = ant_maternos['oligoamnio'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Diabetes mellitus
    ant_maternos['dm'] = ant_maternos['dm'].replace(
    ...
    # HIV
    ant_maternos['hiv'] = ant_maternos['hiv'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Hepatite B
    ant_maternos['hep_b'] = ant_maternos['hep_b'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Hepatite C
    ant_maternos['hep_c'] = ant_maternos['hep_c'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Sífilis - VDRL materno
    ant_maternos['sifilis'] = ant_maternos['sifilis'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Toxoplasmose
    ant_maternos['toxoplasmose'] = ant_maternos['toxoplasmose'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Citomegalovirus
    ant_maternos['cmv'] = ant_maternos['cmv'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Rubéola
    ant_maternos['rubeola'] = ant_maternos['rubeola'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # COVID-19 materno
    ant_maternos['covid_mae'] = ant_maternos['covid_mae'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )

    # Corticóide antenatal
    ant_maternos['cort_antenatal'] = ant_maternos['cort_antenatal'].replace(
        ['1', '2', '3', '', '0'], ['Sim', 'Não', 'Desconhecido', 'Sem registro', 'Sem registro']
    )
...
def exportAllMergedDatasets():
    ...
    for dataset in df_names:
        if dataset == 'ident':
            ...
            df_new = df_old.merge(dfs[dataset], left_index=True, right_index=True)
            df_old = df_new
